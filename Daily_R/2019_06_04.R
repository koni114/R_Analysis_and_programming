...
#  T-TEST, ANOVA
# 이분산 모수 검정
# Welch's ANOVA,  Brown Forsythe test

# 1. ANOVA 전에 Levene 의 등분산 검정을 시행한다.
# 2. 등분산 만족하면 ANOVA, 만족하지 못하면 Welch's ANOVA
...
df <- iris
colnames(iris)
sYvar <- c("Sepal.Length") # 수치형이 종속 변수로 들어간다
sXvar <- c("Species")
MU <- mean(df[,sYvar] - 1)

cat("Y :", sYvar, " X : ", sXvar, " MU : ", MU)

# ** 2019_06_04
# 등분산 모수 검정

# check 사항
# 요인 수준 1개 이하                 -> 분석 요인에서 제외
# 해당 수준 별 개수가 1개 이하       -> 분석 요인에서 제외
# 수준 별 종속변수 값이 동일한 경우, -> 분석 요인에서 제외

# 1. T-TEST : one-sample T-Test(일표본 T-Test)
# 불량 실적, 양품 평균 (양품, MU)
tTest <- t.test(df[,sYvar]
                , alternative = "two.sided"
                , mu=MU, confLevel = confLevel)
tTest$p.value
tTest$statistic
tTest$method

# 2. T-Test : Two-sample T-Test(이표본 T-Test) 
TwoSampletTest <- t.test(iris[,c("Sepal.Length")]
                         ,iris[,c("Petal.Length")]
                         , mu=0
                         ,var.equal = F
                         ,conf.level = 0.95
                         ,alternative = "two.sided")
TwoSampletTest$p.value
...
# 분산분석의 기본 가정이 잘 만족할떄 Kruskal-Wallis test는 일원배치 분산분석에 비해 검정력이 다소 낮음
...
# ** 궁금한 점
# -> T-test, ANOVA 는 종속변수가 수치형, 독립변수가 범주형이어야 하는 것은 아닌가?